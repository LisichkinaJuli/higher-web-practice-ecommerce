...
export function CheckoutPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"card_online" | "cash">(
    "card_online",
  );
  ...
  const handleOrderSubmit = async (e: SyntheticEvent) => {
    e.preventDefault();
    setPhoneError("");
    setAddressError("");
    setAuthError("");
    ...
    const structuredAddress =
      deliveryMethod === "courier"
        ? {
            country: "Россия",
            city: city === "spb" ? "Санкт-Петербург" : "Москва",
            street: address.trim(),
            house: "1",
          }
        : undefined;
    ...
    const newOrder: Order = {
      ...
      items: cartItems.map((item: CartItem) => {
        ...
        if (
          "count" in item &&
          typeof (item as unknown as { count: number }).count === "number"
        ) {
          finalQuantity = (item as unknown as { count: number }).count;
        } else if (
          "quantity" in item &&
          typeof (item as unknown as { quantity: number }).quantity === "number"
        ) {
        ...
        const hasImages =
          Array.isArray(productWithImgs.images) &&
          productWithImgs.images.length > 0;
    ...
    try {
      ...
      setAuthError("Не удалось сохранить заказ на сервере. Попробуйте позже.");
    }
  };
    ...
    <div className="checkout-page">
      ...
      >
        <div className="checkout-page__form-blocks">
          {authError && (
            <div
              className="checkout-page__auth-error"
              style={{
          ...
          <DeliveryBlock
            ...
            pickupIndex={activePickupIndex}
            onPickupIndexChange={setActivePickupIndex}
          />
          ...
          <div className="checkout-summary__card">
            <div className="checkout-summary__rows">
              <div className="checkout-summary__line">
                <span className="checkout-summary__label">Ваш заказ</span>
                <span className="checkout-summary__value">
                  {formatItemsWord(totalCount)}
                </span>
              </div>
              <div className="checkout-summary__line">
                <span className="checkout-summary__label">Сумма заказа</span>
                <span className="checkout-summary__value">
              ...
              <div className="checkout-summary__line">
                ...
                <span className="checkout-summary__value checkout-summary__value_color_success">
                  бесплатно
                </span>
              </div>
              <div className="checkout-summary__divider" />
              <div className="checkout-summary__line checkout-summary__line_type_total">
                <span className="checkout-summary__label_bold">Итого</span>
                <span className="checkout-summary__price-total">
                  {totalPrice.toLocaleString("ru-RU")} ₽
                </span>
              </div>
            </div>
